report 50100 "B2 Invoice"
{
    RDLCLayout = 'report/rdlc/B2SalesInvoice.rdlc';
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(ShowCondition; ShowConditionG)
            {
            }
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                column(CrossReferenceNo_Line; line."Item Reference No.")
                {
            ...
            dataitem(Condition; "Integer")
            {
                DataItemTableView = SORTING(Number) WHERE(Number = CONST(1));
                column(ConditionText; ConditionTextG)
                {
                }
            }
            ...
            begin
                ...
                ConditionTextG := '';
                B2TextL.Reset();
                B2TextL.SetRange(code, 'COND');
                if Header."Language Code" <> '' then
                    B2TextL.SetRange("Language Code", "Language Code")
                else
                    B2TextL.SetRange("Language Code", 'FRS');
                If B2TextL.FindFirst() then Begin
                    ConditionTextG := B2TextL.GetDescription();
                End;
                ShowConditionG := ConditionTextG <> '';
            end;
